...
public class SquareTest
{
	...
	{
		Scanner scan = new Scanner(new File("/Users/raymond_kim/Desktop/FINAL/SquareTest/src/magicData.txt"));
		...
		while (size != -1) {
			...
			square.readSquare(scan);// call its read method to read the values of the square (read in file)
			System.out.println("\n******** Square " + count + " ********");
            square.printSquare();// print the square
            System.out.println();
            // print the sums of its rows
            for(int row = 0; row < size; row++) {
            		System.out.println("The sum of row " + (row + 1) + " is " + square.rowSum(row));
            }
            System.out.println();
